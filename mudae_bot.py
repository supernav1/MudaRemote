...
SPHERE_EMOJIS = ['sp', 'spP', 'spB', 'spT', 'spG', 'spY', 'spO', 'spR', 'spW', 'spL', 'spD', 'spP2', 'spB2', 'spT2', 'spG2', 'spY2', 'spO2', 'spR2', 'spW2', 'spL2', 'spD2', 'spU', 'spM', '🕯️']
...
def is_green_button(btn):
    ...
    if style == 3:
        return True
    return getattr(style, "value", None) == 3
...
def run_bot(token, prefix, target_channel_id, roll_command, min_kakera, delay_seconds, mudae_prefix,
    ...
    def get_maintenance_remaining_seconds() -> float:
        until = getattr(client, "maintenance_until", None)
        if not until:
            return 0.0
    ...
    async def wait_for_maintenance_clear():
        remaining = get_maintenance_remaining_seconds()
        if remaining <= 0:
            return False
    ...
    async def send_roll_command(channel, command_name):
        await wait_for_maintenance_clear()
    ...
    async def check_status(client, channel, mudae_prefix, proceed_to_rolls: bool = True):
        ...
        for _ in range(3):
            ...
            async for msg in channel.history(limit=3):
                if msg.author.id == TARGET_BOT_ID and msg.content:
                    if msg.created_at < tu_requested_at:
                        continue
                    c = msg.content.lower()
        ...
        dk_used_from_this_tu = False
        if client.dk_power_management and client.rolling_enabled:
            dk_used_from_this_tu = await handle_dk_power_management(client, channel, tu_message_content)
        ...
        try:
            ...
            if power_match and not dk_used_from_this_tu:
    ...
    async def check_rolls_left_tu(client, channel, mudae_prefix, log_function, preset_name,
        ...
        if main_match:
            ...
            client.bonus_rolls_this_session = 0  # This is a safeguard for bonus rolls that might spawn on one of the last rolls, right before a $tu 
    ...
    async def start_roll_commands(client, channel, rolls_left, ignore_limit_for_post_roll, key_mode_only_kakera_for_post_roll):
        ...
        if client.bonus_rolls_this_session > 0:
            ...
            await asyncio.sleep(3)  # Give any pending mid-roll $dk activation time to complete first
            client.is_actively_rolling = True
            client.interrupt_rolling = False
        ...
        if is_kakera:
            ...
            has_free_button = msg.components and any(
                hasattr(b.emoji, 'name') and (
                    ...
                    (
                        is_green_button(b) and
                        (b.emoji.name in free_green_targets or b.emoji.name.rstrip('2') in free_green_targets)
                    )
                )
                for c in msg.components
                for b in c.children
            ...
            if cooldown_active and not has_free_button and chaos_count == 0 and not has_sphere_perk:
                return False
                ...
                for comp in msg.components:
                    for btn in comp.children:
                         if hasattr(btn.emoji, 'name') and btn.emoji.name:
                             ...
                             base_emoji_name = emoji_name.rstrip('2')
                             is_target = emoji_name in target_list or base_emoji_name in target_list
                             is_free_green_target = (
                ...
                for btn in all_raw_buttons:
                    ...
                    for attempt in range(2):
                        try:
                            ...
                            log_function(f"[{client.muda_name}] Kakera click failed (attempt {attempt + 1}/2): {char_name} - {e}", client.preset_name, "ERROR")
                            if attempt == 0:
                                await asyncio.sleep(0.5)
                    ...
                    if click_succeeded:
                        ...
                        client._last_kakera_click_ts = time.time()
                        await asyncio.sleep(0.5)
                        # Check immediately in case this click dropped us below threshold
                        if get_current_dk_power() < getattr(client, "dk_activation_percent", dk_activation_percent):
                            await maybe_use_dk_mid_roll(client, channel)
    ...
    async def on_message(message):      
        ...
        # Handle Command Maintenance before embed/character filtering.
        # Mudae sends this as plain message content, not necessarily as an embed description.
        maintenance_parts = [message.content or ""]
        if message.embeds:
            maintenance_embed = message.embeds[0]
            maintenance_parts.extend([
                maintenance_embed.title or "",
                maintenance_embed.description or "",
                maintenance_embed.footer.text if maintenance_embed.footer else "",
            ])
            for field in maintenance_embed.fields:
                maintenance_parts.extend([field.name or "", field.value or ""])
        ...
        if "command under maintenance" in desc:
            ...
            wait_seconds = 300 + random.randint(0, 30)
            client.maintenance_until = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=wait_seconds)
        ...
        # Reactive Kakera on spawned characters (from kakera reactions) - click eligible/free green buttons
        if process and client.is_actively_rolling:
            owner = get_character_owner(embed)
            is_own_character = owner and owner == client.user.name.lower()
            is_spawned_message = message.type != discord.MessageType.chat_input_command

            if is_own_character and is_spawned_message:
                log_function(f"[{client.muda_name}] Kakera spawned via Chaos Kakera: {embed.author.name}!", preset_name, "KAKERA")
                all_k = client.kakera_emojis + client.chaos_emojis + client.sphere_emojis + client.starwish_emojis
                has_btn = False
                if message.components:
                    for c in message.components:
                        for b in c.children:
                            if hasattr(b.emoji, 'name') and b.emoji.name:
                                e_name = b.emoji.name
                                if e_name in all_k or e_name.rstrip('2') in all_k or is_green_button(b):
                                    has_btn = True; break
                        if has_btn: break
            ...
            if is_own_character and client.debug_mode:
                log_function(
                    f"[{client.muda_name}] [MSG DEBUG] type={message.type} "
                    f"interaction_metadata={getattr(message, 'interaction_metadata', 'MISSING')} "
                    f"interaction={getattr(message, 'interaction', 'MISSING')} "
